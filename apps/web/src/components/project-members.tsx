...
import { api, ApiError, type InviteMemberResponse, type MembersList } from "@/lib/api";
...
export function ProjectMembers({ slug }: { slug: string }) {
  ...
  async function invite(event: FormEvent) {
    ...
    setInviting(true);
    try {
      const result = await api.post<InviteMemberResponse>(`/projects/${slug}/invitations`, { email });
      toast.success(
        result.status === "added"
  ...
  async function revoke(id: string) {
    try {
      await api.delete(`/projects/${slug}/invitations/${id}`);
      await reload();
    } catch (error) {

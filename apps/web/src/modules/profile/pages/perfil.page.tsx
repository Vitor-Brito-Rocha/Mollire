...
import { type CurrentUser, setSessionUser } from "@/modules/auth";

const GITHUB_INSTALL_URL = `https://github.com/apps/${import.meta.env.VITE_GITHUB_APP_SLUG}/installations/new`;
...
function Profile() {
    ...
    http
      ...
      .then((me) => {
        ...
        if (me.github_connected) {
          http.get<GithubAccount[]>("/github/accounts").then(setGithubAccounts).catch(() => {});
        }
      })
  ...
  async function handleDisconnect(installationId: string) {
    ...
    try {
      await http.delete(`/github/install/${installationId}`);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const updated = await http.patch<CurrentUser>("/users/me", { handle });
      setUser(updated);
      setSessionUser(updated); // header avatar shares the cached session

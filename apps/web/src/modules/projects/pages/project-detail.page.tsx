...
import { http, API_URL, ApiError } from "@/shared/lib/http";
...
export default function ProjectDetailPage() {
  ...
  async function handleDeploy(commitSha?: string) {
    setDeploying(true);
    try {
      await http.post(`/projects/${slug}/deploy`, commitSha ? { commit_sha: commitSha } : undefined);
  ...
  async function handleToggleVisibility(isPublic: boolean) {
    ...
    try {
      const updated = await http.patch<Project>(`/projects/${slug}/visibility`, {
        is_public: isPublic,
      });

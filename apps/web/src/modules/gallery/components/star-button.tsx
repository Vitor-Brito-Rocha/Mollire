...
function StarIcon({ filled, pending }: { filled: boolean; pending: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      className={"size-[15px] shrink-0 " + (pending ? "animate-pulse" : "")}
...
export function StarButton({
  ...
  return (
    <Button
      ...
      className={
        "font-display min-h-11 w-fit gap-2 px-3.5 text-xs font-semibold tracking-[0.08em] disabled:opacity-100 " +
        (starred
    ...
    >
      <StarIcon filled={starred} pending={pending} />

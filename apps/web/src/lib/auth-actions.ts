...
// The API is called straight from the browser with the cookie, and it can't
// refresh a token (only the Next side writes the cookie). When the access
// token expires mid-page the API answers 401 and the client calls this, whose
// getUser() refreshes the session and re-sets the cookie, then retries.

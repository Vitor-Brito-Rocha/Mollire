...
const turmas = [
  { id: "t1", name: "Turma 3B — Projetos Web", code: "K7XQ2M", description: "Sites estáticos do segundo bimestre. Cada grupo entrega um projeto.", is_public: false, capacity: 40, group_mode: "GROUPS", owner_id: "u-voce", owner: "voce", created_at: h(24 * 9),
    groups: [{ id: "grp1", name: "Grupo 1", max_size: 4, code: "M4K2PX" }, { id: "grp2", name: "Grupo 2", max_size: 4, code: "T7V3QW" }, { id: "grp3", name: "Grupo 3", max_size: 3, code: "B9N6ZR" }],
    ...
    projects: [{ slug: "clima-agora", group_id: "grp1", grade: 8.5, graded_at: h(24), comment: "Bom uso do grid; falta o estado vazio quando a cidade não existe.", comment_public: false }, { slug: "cobrinha", group_id: "grp1", grade: null, graded_at: null, comment: null, comment_public: false }, { slug: "senha-forte", group_id: "grp2", grade: 9, graded_at: h(30), comment: "README exemplar. Deem uma olhada em como ele explica o projeto.", comment_public: true }],
    milestones: [
      { id: "ms1", title: "Site no ar", description: "Qualquer página, mas publicada de verdade.", due_at: inDays(-3), requirements: { deployed: true, published: false, min_stars: null }, created_at: h(24 * 8), done: { grp1: inDays(-5), grp2: inDays(-1) } },
      { id: "ms2", title: "Na galeria", description: "Projeto público, com capa.", due_at: inDays(6), requirements: { deployed: true, published: true, min_stars: null }, created_at: h(24 * 8), done: { grp1: inDays(-2) } },
      { id: "ms3", title: "Três estrelas", description: null, due_at: inDays(20), requirements: { deployed: true, published: true, min_stars: 3 }, created_at: h(24 * 7), done: {} },
    ] },
  { id: "t2", name: "Curso de React", code: "R3ACTX", description: "Do zero ao deploy em oito semanas.", is_public: true, capacity: null, group_mode: "GROUPS", owner_id: "u-ana", owner: "prof.ana", created_at: h(24 * 30),
    groups: [{ id: "grp4", name: "Equipe Norte", max_size: 5, code: "H2C8DF" }, { id: "grp5", name: "Equipe Sul", max_size: 5, code: "W5J4TY" }],
    ...
    projects: [{ slug: "receitas-da-vo", group_id: "grp4", grade: 9.5, graded_at: h(48), comment: "Ótima escolha de paleta. Sirva de referência para a turma.", comment_public: true }, { slug: "marina-dev", group_id: "grp5", grade: null, graded_at: null, comment: null, comment_public: false }, { slug: "turma-3b", group_id: null, grade: 7.5, graded_at: h(10), comment: "Faltou o responsivo no celular; o resto está redondo.", comment_public: false }],
    milestones: [
      { id: "ms4", title: "Site no ar", description: "Semana 3.", due_at: inDays(-10), requirements: { deployed: true, published: false, min_stars: null }, created_at: h(24 * 25), done: { grp4: inDays(-12) } },
      { id: "ms5", title: "Publicado na galeria", description: "Semana 6: público, com capa e descrição.", due_at: inDays(4), requirements: { deployed: true, published: true, min_stars: null }, created_at: h(24 * 25), done: {} },
    ] },
...
const newCode = () => Math.random().toString(36).replace(/[01ilo]/g, "").slice(2, 8).toUpperCase().padEnd(6, "X");
const groupView = (t, g) => ({ id: g.id, name: g.name, max_size: g.max_size, members_count: t.members.filter((m) => m.group_id === g.id).length, code: g.code });
// Níveis fictícios para as insígnias da equipe.
const LEVELS = { voce: 9, dudu: 4, "helena.r": 7, "kauan.t": 5, "bia.m": 3, "marina.s": 6, "prof.aline": 9, "prof.ana": 11, "prof.carlos": 8 };
const FRAMES = { "helena.r": "silver", "prof.ana": "gold", "marina.s": "bronze" };
const teamOf = (t, s, author) => { const mates = s.group_id ? t.members.filter((m) => m.group_id === s.group_id).map((m) => m.handle) : []; return [author, ...mates.filter((x) => x !== author)].map((handle) => ({ handle, level: LEVELS[handle] ?? 3, frame: FRAMES[handle] ?? null })); };
// Entregas: unidades (grupos ou alunos), projeto de cada uma, estado por prazo.
const unitsOf = (t) => (t.group_mode === "GROUPS" ? t.groups.map((g) => ({ type: "group", id: g.id, name: g.name })) : students(t).map((s) => ({ type: "student", id: s.user_id, name: s.handle })));
const unitProject = (t, unit) => { const s = unit.type === "group" ? t.projects.find((p) => p.group_id === unit.id) : t.projects.find((p) => authorOf(p.slug) === unit.name); const p = s && projectOf(s.slug); return p ? { slug: p.slug, name: p.name } : null; };
const msStatus = (ms, unitId) => { const done = ms.done[unitId]; const due = new Date(ms.due_at).getTime(); if (done) return { status: new Date(done).getTime() <= due ? "done" : "late", completed_at: done }; return { status: Date.now() > due ? "missed" : "pending", completed_at: null }; };
const myUnit = (t) => { const m = memberOf(t); if (!m || m.role === "PROFESSOR") return null; if (t.group_mode === "GROUPS") { const g = t.groups.find((x) => x.id === m.group_id); return g ? { type: "group", id: g.id, name: g.name } : null; } return { type: "student", id: m.user_id, name: m.handle }; };
const milestoneView = (t, ms) => { const base = { id: ms.id, title: ms.title, description: ms.description, due_at: ms.due_at, requirements: ms.requirements, created_at: ms.created_at }; if (memberOf(t).role === "PROFESSOR") { const units = unitsOf(t); return { ...base, mine: null, stats: { completed: units.filter((u) => ms.done[u.id]).length, total: units.length } }; } const unit = myUnit(t); return { ...base, mine: unit ? { ...msStatus(ms, unit.id), project: unitProject(t, unit) } : null, stats: null }; };
const milestoneRows = (t, ms) => unitsOf(t).map((u) => ({ unit: u, project: unitProject(t, u), ...msStatus(ms, u.id) }));
const turmaProgress = (t) => { const st = students(t); const units = unitsOf(t); const withDeploy = st.filter((s) => (t.group_mode === "GROUPS" ? s.group_id && t.projects.some((p) => p.group_id === s.group_id) : t.projects.some((p) => authorOf(p.slug) === s.handle))).length; const published = t.projects.filter((p) => { const x = projectOf(p.slug); return x && (x.is_public ?? true); }).length; return { students: st.length, with_deploy: withDeploy, published, milestones: (t.milestones ?? []).map((ms) => ({ id: ms.id, title: ms.title, completed: units.filter((u) => ms.done[u.id]).length, total: units.length })), xp_week: 1240 }; };
...
const turmaGallery = (t, auth) => t.projects.map((s) => { const p = projectOf(s.slug); const g = t.groups.find((x) => x.id === s.group_id); const author = authorOf(s.slug); const team = teamOf(t, s, author); const m = memberOf(t); const professor = m?.role === "PROFESSOR"; const onTeam = team.some((x) => x.handle === me.handle); const reader = professor || onTeam || !!s.comment_public; return { ...card({ ...p, author, stars: p.stars ?? 0 }, auth), group: g ? { id: g.id, name: g.name } : null, grade: s.grade, graded_at: s.graded_at, team, ...(reader ? { grade_comment: s.comment ?? null } : {}), ...(professor || onTeam ? { grade_comment_public: !!s.comment_public } : {}) }; });
...
http.createServer(async (req, res) => {
  ...
  if (path === "/xp/rules") return send(200, [{ code: "DEPLOY", xp: 10 }, { code: "FIRST_DEPLOY", xp: 50 }, { code: "PUBLISH", xp: 20 }, { code: "STAR_RECEIVED", xp: 5 }, { code: "QUEST", xp: 10 }, { code: "ACHIEVEMENT", xp: 25 }, { code: "HELPFUL_COMMENT", xp: 5 }, { code: "TEAM_DEPLOY", xp: 3 }, { code: "MILESTONE", xp: 30 }]);
  const achievements = [{ code: "first_deploy", unlocked_at: h(46) }, { code: "ten_deploys", unlocked_at: null, progress: { current: 6, target: 10 } }, { code: "first_star", unlocked_at: h(20) }, { code: "fast_deploy", unlocked_at: h(5) }, { code: "rollback", unlocked_at: null }, { code: "collaborator", unlocked_at: null, progress: { current: 1, target: 3 } }, { code: "uptime_30", unlocked_at: null, progress: { current: 12, target: 30 } }, { code: "team_all_deployed", unlocked_at: null, progress: { current: 2, target: 3 } }, { code: "team_first_milestone", unlocked_at: h(1) }];
  ...
  if (path === "/turmas/join") { const code = String(body.code).toLowerCase(); let t = turmas.find((x) => x.code.toLowerCase() === code); let group = null; if (!t) { t = turmas.find((x) => x.groups.some((g) => g.code?.toLowerCase() === code)); group = t?.groups.find((g) => g.code?.toLowerCase() === code) ?? null; } if (!t) return send(404, { message: "código não encontrado" }); let mem = memberOf(t); if (!mem) { if (t.capacity !== null && students(t).length >= t.capacity) return send(409, { message: "turma cheia" }); mem = { user_id: me.id, handle: me.handle, role: "ALUNO", group_id: null, joined_at: new Date().toISOString() }; t.members.push(mem); } let groupFull = false; if (group && mem.role === "ALUNO") { if (t.members.filter((x) => x.group_id === group.id).length >= group.max_size && mem.group_id !== group.id) groupFull = true; else mem.group_id = group.id; } return send(200, { turma_id: t.id, role: mem.role, group_id: mem.group_id, ...(groupFull ? { group_full: true } : {}) }); }
  ...
  // --- ideias de grupos (docs/api-grupos.md): progresso e entregas ---
  if ((m = path.match(/^\/turmas\/([^/]+)\/progress$/))) { const t = turmaFor(m[1]); if (!t) return send(404, {}); return send(200, turmaProgress(t)); }
  if ((m = path.match(/^\/turmas\/([^/]+)\/milestones$/))) { const t = turmaFor(m[1]); if (!t) return send(404, {}); t.milestones ??= []; if (req.method === "POST") { if (memberOf(t).role !== "PROFESSOR") return send(404, {}); const ms = { id: "ms" + Date.now(), title: String(body.title), description: body.description ?? null, due_at: body.due_at, requirements: { deployed: !!body.requirements?.deployed, published: !!body.requirements?.published, min_stars: body.requirements?.min_stars ?? null }, created_at: new Date().toISOString(), done: {} }; t.milestones.push(ms); t.milestones.sort((a, b) => a.due_at.localeCompare(b.due_at)); return send(201, milestoneView(t, ms)); } return send(200, [...t.milestones].sort((a, b) => a.due_at.localeCompare(b.due_at)).map((ms) => milestoneView(t, ms))); }
  if ((m = path.match(/^\/turmas\/([^/]+)\/milestones\/([^/]+)$/))) { const t = turmaFor(m[1]); if (!t || memberOf(t).role !== "PROFESSOR") return send(404, {}); const ms = (t.milestones ?? []).find((x) => x.id === m[2]); if (!ms) return send(404, {}); if (req.method === "DELETE") { t.milestones = t.milestones.filter((x) => x.id !== ms.id); return send(204); } if (req.method === "PATCH") { if (body.title !== undefined) ms.title = String(body.title); if (body.description !== undefined) ms.description = body.description; if (body.due_at !== undefined) ms.due_at = body.due_at; if (body.requirements) ms.requirements = { deployed: !!body.requirements.deployed, published: !!body.requirements.published, min_stars: body.requirements.min_stars ?? null }; return send(200, milestoneView(t, ms)); } return send(200, { ...milestoneView(t, ms), rows: milestoneRows(t, ms) }); }
  ...
  if ((m = path.match(/^\/turmas\/([^/]+)\/groups$/))) { const t = turmaFor(m[1]); if (!t || memberOf(t).role !== "PROFESSOR") return send(404, {}); if (t.groups.some((g) => g.name.toLowerCase() === String(body.name).toLowerCase())) return send(409, { message: "já existe um grupo com esse nome" }); const g = { id: "grp" + Date.now(), name: body.name, max_size: body.max_size, code: newCode() }; t.groups.push(g); t.group_mode = "GROUPS"; return send(201, groupView(t, g)); }
  ...
  if ((m = path.match(/^\/turmas\/([^/]+)\/projects\/([^/]+)\/grade$/))) { const t = turmaFor(m[1]); if (!t || memberOf(t).role !== "PROFESSOR") return send(404, {}); const s = t.projects.find((x) => x.slug === m[2]); if (!s) return send(404, {}); s.grade = Number(body.grade); s.graded_at = new Date().toISOString(); if ("comment" in body) s.comment = body.comment ? String(body.comment).slice(0, 2000) : null; if ("comment_public" in body) s.comment_public = !!body.comment_public; return send(200, {}); }

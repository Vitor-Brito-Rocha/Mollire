...
- **`uptime_30` também manda push:** ela só fica verdadeira com o tempo passando, então um job diário às **00:45** (fuso do servidor, `TZ`; troca com `UPTIME_MILESTONE_AT=HH:MM`; `progress/uptime-milestone.service.ts`, mesmo liga/desliga do ping) avalia só quem tem um site publicado com check de 30+ dias e ainda não a desbloqueou.
